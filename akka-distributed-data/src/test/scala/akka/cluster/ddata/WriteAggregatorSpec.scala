...
object WriteAggregatorSpec {
  ...
  def writeAggregatorProps(data: GSet[String], consistency: Replicator.WriteConsistency,
                           probes: Map[UniqueAddress, ActorRef], nodes: Set[UniqueAddress], unreachable: Set[UniqueAddress], replyTo: ActorRef, durable: Boolean): Props =
  ...
  class TestWriteAggregator(key: Key.KeyR, data: ReplicatedData, delta: Option[Delta], consistency: Replicator.WriteConsistency,
                            probes: Map[UniqueAddress, ActorRef], nodes: Set[UniqueAddress], unreachable: Set[UniqueAddress], replyTo: ActorRef, durable: Boolean)
