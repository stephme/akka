  ...
  override def behavior: Behavior[Command] =
    Behaviors.setup { ctx ⇒
      ...
      Behaviors.withTimers { timers ⇒
        ...
        val clusterEventMessageAdapter: ActorRef[MemberEvent] =
          ctx.messageAdapter[MemberEvent] {
            ...
            case other                    ⇒ throw new IllegalStateException(s"Unexpected MemberEvent $other. Please report bug.")
          }
        setup.cluster.subscribe(clusterEventMessageAdapter.toUntyped, ClusterEvent.InitialStateAsEvents,
      ...
      def onCommand(cmd: Command): Behavior[Command] = cmd match {
        case ReceptionistMessages.Register(key, serviceInstance, maybeReplyTo) ⇒
          if (serviceInstance.path.address.hasLocalScope) {
            ...
            ctx.log.debug("ClusterReceptionist [{}] - Actor was registered: [{}] [{}]", cluster.selfAddress,
              key, entry)
            ...
            ctx.log.error(
              s"ClusterReceptionist [{}] - Register of non-local [{}] is not supported",
              serviceInstance)
            Behaviors.same
          }
        ...
        case ReceptionistMessages.Subscribe(key, subscriber) ⇒
          ...
          val listing = ReceptionistMessages.Listing(
            key.asServiceKey,
            registry.activeActorRefsFor(key, selfUniqueAddress))
          subscriber ! listing
      ...
      def onInternalCommand(cmd: InternalCommand): Behavior[Command] = cmd match {
        ...
        case RegisteredActorTerminated(key, serviceInstance) ⇒
          ...
          ctx.log.debug(
            ...
            cluster.selfAddress, key.asServiceKey.id, entry)
          val ddataKey = registry.ddataKeyFor(key)
          replicator ! Replicator.Update(ddataKey, EmptyORMultiMap, settings.writeConsistency) { registry ⇒
          ...
          if (changedKeys.nonEmpty) {
            ...
            changedKeys.foreach { changedKey ⇒
              ...
              if (tombstonedButReAdded.nonEmpty) {
                if (ctx.log.isDebugEnabled) ctx.log.debug(
                  "ClusterReceptionist [{}] - Saw ActorRefs that were tomstoned [{}], re-removing.",
                  cluster.selfAddress, tombstonedButReAdded.mkString(", "))

                replicator ! Replicator.Update(ddataKey, EmptyORMultiMap, settings.writeConsistency) { registry ⇒
                  tombstonedButReAdded.foldLeft(ServiceRegistry(registry)) { (acc, ref) ⇒
              ...
              if (removedRegisteredServices.nonEmpty) {
                ctx.log.error(
                  ...
                  cluster.selfAddress, removedRegisteredServices.mkString(", "))
        ...
        case NodeRemoved(uniqueAddress) ⇒
          if (uniqueAddress == selfUniqueAddress || cluster.isTerminated) {
            ...
            if (isLeader) {
              ctx.log.debug(
                ...
                cluster.selfAddress, uniqueAddress)
            ...
            if (notInCluster.isEmpty) Behavior.same
              ...
              if (ctx.log.isDebugEnabled)
                ctx.log.debug(
                  ...
                  cluster.selfAddress, notInCluster.mkString(","))
