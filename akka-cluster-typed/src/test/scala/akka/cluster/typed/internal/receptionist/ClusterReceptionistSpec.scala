    ...
    "eventually replicate registrations to the other side" in {
      ...
      try {
        ...
        regProbe1.awaitAssert(
          clusterNode1.state.members.count(_.status == MemberStatus.Up) should ===(2),
          10.seconds)
    ...
    "handle a new incarnation of the same node well" in {
      ...
      try {
        ...
        try {
          ...
          system1.log.debug("Starting system3 at same hostname port as system2: [{}]", clusterNode3.selfMember.uniqueAddress)
